...
def read_config():
    ...
    config['dataset_path'] = os.path.abspath(os.path.expanduser(config['dataset_path']))
    config['output_path'] = os.path.abspath(os.path.expanduser(config['output_path']))
    config['hdf5_file_path'] = os.path.join(config['output_path'], 'pairs.hdf5')
...
def load_paths(config,sequence_name):
    ...
    config['seq_read_path'] = os.path.join(config['dataset_path'], sequence_name)
    config['img_read_path'] = os.path.join(config['seq_read_path'], 'img')
    config['depth_read_path'] = os.path.join(config['seq_read_path'], 'depth')
    config['intrinsics_path'] = os.path.join(config['seq_read_path'], 'intrinsics.txt')
    config['poses_path'] = os.path.join(config['seq_read_path'], config['poses_file'])
...
def save_pairs_to_hdf5(hdf, pairs, hdf5_folder):
    ...
    for pair in pairs:
        ...
        rel_img_paths = (make_relative(pair['img_paths'][0]), make_relative(pair['img_paths'][1]))
        pair_group.create_dataset('img_paths', data=np.array(rel_img_paths, dtype=h5py.string_dtype()))
        pair_group.create_dataset('points1', data=np.array(pair['points1']))
        pair_group.create_dataset('pos_points2', data=np.array(pair['pos_points2']))
        pair_group.create_dataset('neg_points2', data=np.array(pair['neg_points2']))
...
def save_sampled_pairs(pairs, demo_path):
    ...
    for i, pair in enumerate(sampled_pairs):
        ...
        img1 = cv2.cvtColor(cv2.imread(img1_path, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
        img2 = cv2.cvtColor(cv2.imread(img2_path, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
        ...
        for p1, p2 in zip(pair['points1'], pair['pos_points2']):
            ...
            ax1.scatter(p2[0] + img1.shape[1], p2[1], color='g', s=10)  # Offset by img1 width for alignment
...
def count_img_paths(hdf):
    ...
    for pair_name, pair_group in pairs_group.items():
        img_paths = pair_group['img_paths'][:]  # Read the entire dataset into memory
        num_img_paths = len(img_paths)  # Count the number of image paths in the dataset
