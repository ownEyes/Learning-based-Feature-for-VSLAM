...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    mpORBextractorLeft = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST,modelPath);

    if(mSensor==System::STEREO || mSensor==System::IMU_STEREO)
        mpORBextractorRight = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST,modelPath);

    if(mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR)
        mpIniORBextractor = new ORBextractor(5*nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST,modelPath);
...
bool Tracking::ParseORBParamFile(cv::FileStorage &fSettings)
{
    ...
    node = fSettings["ORBextractor.modelPath"]; // Reading the new string parameter
    if (!node.empty() && node.isString())
    {
        modelPath = node.string();
    }
    else
    {
        std::cerr << "*ORBextractor.modelPath parameter doesn't exist or is not a string*" << std::endl;
        b_miss_params = true;
    }
...
bool Tracking::ParseIMUParamFile(cv::FileStorage &fSettings)
{
    ...
    float Ng=0.0, Na=0.0, Ngw=0.0, Naw=0.0;
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    {
        if (mSensor == System::IMU_MONOCULAR && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=0.001)
            return true;
        else if ((mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD) && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=0.1)
            return true;
        else
            return false;
    }
    ...
    const bool c1a = mCurrentFrame.mnId>=mnLastKeyFrameId+mMaxFrames/10;
    ...
    const bool c1c = mSensor!=System::MONOCULAR && mSensor!=System::IMU_MONOCULAR && mSensor!=System::IMU_STEREO && mSensor!=System::IMU_RGBD && (mnMatchesInliers<nRefMatches*0.05 || bNeedToInsertClose) ;
    // Condition 2: Few tracked points compared to reference keyframe. Lots of visual odometry compared to map matches.
    const bool c2 = (((mnMatchesInliers<nRefMatches*thRefRatio || bNeedToInsertClose)) || mnMatchesInliers>1);
    ...
    bool c4 = false;
    if ((((mnMatchesInliers<100) && (mnMatchesInliers>1)) || mState==RECENTLY_LOST) && (mSensor == System::IMU_MONOCULAR)) // MODIFICATION_2, originally ((((mnMatchesInliers<75) && (mnMatchesInliers>15)) || mState==RECENTLY_LOST) && ((mSensor == System::IMU_MONOCULAR)))
        c4=true;
    else
        c4=false;
